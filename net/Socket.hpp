...
public:
    ...
    virtual ssize_t Send(const void *buf, size_t len, int flag) = 0;
...
public:
    ...
    ssize_t Send(const void *buf, size_t len, int flag = 0) override {
        ssize_t ret = send(_sockfd, buf, len, flag);
        if(ret < 0) {
            if(errno == EAGAIN || errno == EINTR) {
                //LOG_INFO << "send EAGAIN or EINTR";
                LOG_TRACE("Socket发送收到 EAGAIN || EINTR");
                return 0;
            }
            //LOG_ERROR << "SOCKET SEND FAILED";
            LOG_ERROR("Socket发送数据失败, errno = {}", errno);
            return -1;
        }
        return ret;
    }
